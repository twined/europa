...
var _advancedBreakpointQuery = _interopRequireDefault(require("./advancedBreakpointQuery"));
  ...
  if (!_lodash.default.has(config.theme.spacing, size)) {
    ...
    if (size.indexOf('/') !== -1) {
      ...
      if (!_lodash.default.has(config.theme.spacing, head)) {
        ...
        if ((0, _advancedBreakpointQuery.default)(bp)) {
          throw node.error('SPACING: No support for advanced breakpoints when using fractions (due to gutter calculations)', {
            name: bp
          });
        }
