  ...
  return function (css) {
    ...
    css.walkAtRules('unpack', atRule => {
      ...
      if (params === 'containerPadding') {
        ...
        const obj = _lodash.default.get(config, ['theme', 'container', 'padding']);
        ...
        _lodash.default.keys(obj).forEach(breakpoint => {
          let value = obj[breakpoint]; // build decls for each k/v

          const decls = [(0, _buildDecl.default)('--container-padding', value)]; // build a responsive rule with these decls
      ...
      if (params === 'gridGutter') {
        // unpack container values to css vars
        const obj = _lodash.default.get(config, ['theme', 'columns', 'gutters']);
        ...
        _lodash.default.keys(obj).forEach(breakpoint => {
          ...
          const decls = [(0, _buildDecl.default)('--grid-gutter', value)]; // build a responsive rule with these decls
