...
var _parseRFSQuery = _interopRequireDefault(require("../../util/parseRFSQuery"));
 ...
 *
 */
var _default = _postcss.default.plugin('europacss-rfs', getConfig => {
  return function (css) {
    const {
      theme
    } = getConfig();
    ...
    css.walkAtRules('rfs', atRule => {
      ...
      if (parent.type === 'atrule' && parent.name === 'responsive') {
        if (bpQuery) {
          throw cp.error(`RFS: When nesting @rfs under @responsive, we do not accept a breakpoints query.`, {
            name: bpQuery
          });
        }
      ...
      if (bpQuery) {
        ...
        _lodash.default.each(affectedBreakpoints, bp => {
          let parsedFontSizeQuery = (0, _parseRFSQuery.default)(cp, theme, fontSizeQuery, bp);
